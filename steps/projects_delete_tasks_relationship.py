...
def step_valid_project_id_with_task_id(context):
    ...
    # # keep creating tasks relationships or projects until project_id = tasks_id
    # if(tasks_id > project_id):
    #     while(project_id != tasks_id):
    #         context.response = sendRequest("POST", "projects", data=TEST_DATA_PROJECT_TASKS, return_response_if_error=True)
...
def step_verify_task_relationship_deleted(context):
    ...
    if projectsGetEntries("projects")[0].get("tasks") is not None:
